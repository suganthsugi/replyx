...
| api-conventions | ready | AppError and error format, validation pipe, cursor pagination, route decorators (`@RequirePermission`/`@Public`/`@CustomerApi`/`@OperatorApi`), `openapi.yaml` operationIds and tags, customer vs staff APIs | backend-agent, frontend-connector, reviewer |
...
| ui-components | ready | MUI theme and tokens, shared component system, accessibility (loading/empty/error, focus, LiveRegion), no ticket concepts in the customer UI | frontend-agent |
...
| web-testing | ready | RTL role queries, typed MSW handlers, axe checks, Playwright projects and Mailpit helpers | frontend-automator |
